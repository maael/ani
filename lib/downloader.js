var torrent = require('./torrent')
  ...
  , db = require('./db')
  , log = require('./logger')({ component: 'downloader' })

function download(eps) {
  eps = eps.map(function (ep) {
    return new Promise(function (resolve, reject) {
      torrent.add(ep.link
      , { path: `${torrent.downloadFolder + ep.series}/` }
      , function (torrent) {
        var previousProgress = 0
        db.updateEpisodeStatus(ep.guid, 'Started')
...
module.exports = function (subs) {
  return new Promise(function (resolve, reject) {
    fetchNewEpisodes(subs)
      .then(db.load)
      .then(db.getEpisodes.bind(null, { status: { $nin: [ 'Started', 'Downloaded' ] } }))
      .then(download)
      .then(resolve)
      .catch(reject)
  })
